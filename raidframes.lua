...
local Compat = nil
do
    local ok, mod = pcall(require, "nuzi-raid/overlay_utils")
    if ok then
        Utils = mod
    else
        ok, mod = pcall(require, "nuzi-raid.overlay_utils")
        if ok then
            Utils = mod
        end
    end
...
do
    local ok, mod = pcall(require, "nuzi-raid/runtime")
    if ok then
        Runtime = mod
    else
        ok, mod = pcall(require, "nuzi-raid.runtime")
        if ok then
            Runtime = mod
        end
    end
...
do
    local ok, mod = pcall(require, "nuzi-raid/compat")
    if ok then
        Compat = mod
    else
        ok, mod = pcall(require, "nuzi-raid.compat")
        if ok then
            Compat = mod
        end
    end
...
if Utils == nil then
    error("[Nuzi Raid] overlay_utils unavailable")
end
